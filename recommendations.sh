...
log_info "Get matches array from vector DB response"
jq '.results[].matches' "$VECTOR_DB_RESPONSE" > "$IN_VECTOR_DB"

# Get the vectorId and templateName
log_info "Get the vectorId and templateName from vector DB response"
jq -r '[
    .[]
...
log_info "Find the vectorId of missing templateName"
